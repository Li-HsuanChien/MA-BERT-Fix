...
class IMDB(SentenceProcessor):
    ...
    def get_polarzied_keywords(self):
        return self._get_polarized_keywords(self.d_train, self.d_dev,
...
class GOOGLE(SentenceProcessor):
    ...
    def __init__(self, data_dir='corpus'):
        self.d_train = self._read_file(os.path.join(data_dir, 'google', 'google.train.ss'))
        self.d_dev = self._read_file(os.path.join(data_dir, 'google', 'google.dev.ss'))
        self.d_test = self._read_file(os.path.join(data_dir, 'google', 'google.test.ss'))
    ...
    def _create_examples(self, documents, type):
        ...
        for (i, line) in enumerate(documents):
            ...
            if i % 5000 == 0:
                print(text)
            examples.append(
                InputExample(guid=guid, user=None, product=None, text=text, label=int(line[1]), category=None))
        return examples
